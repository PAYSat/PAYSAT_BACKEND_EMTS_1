  ...
  <body>
    ...
    <script>
      (function () {
        ...
        if (!cardId) {
          document.getElementById('error-message').textContent = 'No se encontró card_id en la URL.';
          return;
        }
        ...
        function showError(message) {
          ...
          el.textContent = message || 'Ocurrió un error cargando la información de la tarjeta.';
        }
          ...
          try {
            ...
            var res = await fetch('/api/issuing/ephemeral-keys', {
              ...
              headers: {
                ...
                'Authorization': window.PAYSAT_JWT ? 'Bearer ' + window.PAYSAT_JWT : '',
              },
              body: JSON.stringify({
                card_id: cardId,
                nonce: nonce,
            ...
            if (!res.ok) {
              console.error('Error HTTP al pedir ephemeral key:', res.status);
              showError('No se pudo obtener autorización para ver la tarjeta.');
              return;
            }
